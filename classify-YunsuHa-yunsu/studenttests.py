...
class TestAbsLoss(unittest.TestCase):
    def test_simple(self):
        ...
        t.check_array(result_array, [1, 1, 1, 1, 15])
    ...
    def test_zeros(self):
        ...
        t.input_array("a3", result_array)
        t.call("abs_loss")
        t.check_array(result_array, [0, 1, 2, 3, 10])
...
class TestSquaredLoss(unittest.TestCase):
    def test_simple(self):
        ...
        t.call("squared_loss")
        # check that the result array contains the correct output
        t.check_array(result_array, [0, 4, 9, 16, 25])
        # check that the register a0 contains the correct output
...
class TestZeroOneLoss(unittest.TestCase):
    def test_simple(self):
        ...
        # create input arrays in the data section
        array0 = t.array([0, 2, -12, 4, -5, 0])
        array1 = t.array([1, 2, -12, 6, 5, 0])
        # load array addresses into argument registers
        t.input_array("a0", array0)
        t.input_array("a1", array1)
        # load array length into argument register
        t.input_scalar("a2", 6)
        # create a result array in the data section (fill values with -1)
        result_array = t.array([-1, -1, -1, -1, -1, -1])
        # load result array address into argument register
        t.input_array("a3", result_array)
        # call the `zero_one_loss` function
        t.call("zero_one_loss")
        # check that the result array contains the correct output
        t.check_array(result_array, [0, 1, 1, 0, 0, 1])
